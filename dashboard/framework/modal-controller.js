...
framework.angular.controllers.controller("modalCtrl",['$scope','$rootScope',function($scope,$rootScope){
    //接收打开窗口广播
    $rootScope.$on("openModal",function(event,para){
        ...
        if(_para.width) { 
            $('#main-modal').css({width:_para.width,"margin-left":0,left:( ($(window).width() - _para.width )) /2,'overflow':'hidden','border-width':'0px'  }); 
            $('#main-modal .modal-dialog').css({width:_para.width}); 
        }
        if(_para.height) { $('#modal-body').css({"max-height":_para.height}); }
        var modal_config = {keyboard:_para.keyboard};
        $('#main-modal').css({width:_para.width,"margin-top":0,top:(($(window).height() - _para.height )) /2 -100,'overflow':'hidden','border-width':'0px'  });
...
//                        console.log("onSubmit return false.");
            ...
            // console.info("监听iframePageLoaded，如有接收，则发送广播modal-boardcast",url);
            $rootScope.$on("iframePageLoaded:"+url , function(event,para){
                //console.info("监听到iframePageLoaded，并发送广播modal-boardcast");
...
function __modalSubmitOnWindow(){
    angular.element('#main-modal').scope().modalSubmit();            //触发model提交方法
}