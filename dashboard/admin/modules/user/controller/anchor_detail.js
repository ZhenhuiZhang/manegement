framework.angular.controllers.controller("anchor-detail", ['$scope', '$rootScope', 'commonRES','$filter',
    function ($scope, $rootScope, service, $filter) {
        ...
        $scope.fit_in_locations_handleCheck = function($event){
            $scope.model.fit_in_locations = $scope.model.fit_in_locations || []
            if($scope.model.fit_in_locations.indexOf($($event.target).val()) > -1){
                $scope.model.fit_in_locations.splice($scope.model.fit_in_locations.indexOf($($event.target).val()),1)
            }else{
                $scope.model.fit_in_locations.push($($event.target).val())
            }
        }
