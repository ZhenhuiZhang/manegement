framework.angular.controllers.controller("anchor-list", ['$scope', 'commonRES', '$filter',
    function($scope, service, $filter) {
        var url = framework.getFinalURL('banner/find?content_type=anchor&banner_type=recommend', 'api/user.json')
        ...
        $scope.handlebar_Renderer = function(dataItem){
            var result = '<a href="anchor_recommend_detail.html?id='+ dataItem._id +'">Edit</a> ';
        ...
        $scope.del = function(_id) {
            var _data = getSelectedAllData($scope.recommend_anchor_list_grid);
            if (_data) {
        ...
        $scope.onInitialized = function(grid) {
            ...
            grid.dbclick(function(event) {
                var _data = getSelectedData($scope.recommend_anchor_list_grid);
                if (_data) {
                    location.href = 'anchor_recommend_detail.html?id=' + _data._id;
                }
            });
        }
