framework.angular.controllers.controller("anchor-detail", ['$scope', 'commonRES','$filter',
function($scope, service, $filter) {
   ...
   //日期区间
    ...
    var checkout = $('.dpd2').datepicker({
        onRender: function(date) {
            ...
            if($(".dpd1").val() == $(".dpd2").val()){
                console.log(parseInt(new Date($(".dpd1").val()).getDay()));
                $scope.$apply(function() {
                    $scope.week.splice(0,$scope.week.length);
                    $scope.week[parseInt(new Date($(".dpd1").val()).getDay())]=parseInt(new Date($(".dpd1").val()).getDay())
                });
   ...
   //是否周期重复选项
    ...
    $scope.save = function(){
        ...
        var params = {"_id":$scope.model._id, "banner_type":"recommend",
                        "platform":$scope.model.platform,"platform_version":$scope.model.platform_version,"by_date":$scope.model.by_date,"enable":$scope.model.enable,"content_type":"anchor",
                        "by_week_date":$scope.model.by_week_date,"by_week":$scope.model.by_week,"weight":$scope.model.weight,"pic":$scope.model.pic,"content":$scope.model.content, "title":$scope.model.title
                    };
        ...
        var callback = function (uploadResult) {
            ...
            service.save({
                ...
                if (result.code == 0) {
                    alert('submit ok.');
                    location.href = 'anchor_recommend.html';
                } else {
