...
// custom bar chart
    ...
    //        window.parent.resizeFrame(_height,window.self.frameElement.id);
    //        return;
    //    }
...
$(function(){
    if(window.localStorage.getItem('timezone')){
        var length = $(".timezone").find("option[value='"+window.localStorage.getItem('timezone')+"']").size();
        if(length){
            $('.timezone').val(window.localStorage.getItem('timezone'))
        }
    }else{
        //若未设置过时区，则把时区设为当地时区
        window.localStorage.setItem('timezone',jstz.determine().name())
        // $('.timezone').val(jstz.determine().name())
    }

    $('.timezone').change(function(){
        window.localStorage.setItem('timezone',$('.timezone').val())
