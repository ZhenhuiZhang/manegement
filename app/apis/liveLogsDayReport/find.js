...
module.exports = {
    ...
    executor: function(inputs,res,next, cb, req){
        ...
        cms_report_db.LiveLogsDayReport.aggregate(option).allowDiskUse(true).exec(function(err,rd){
                if(err) cb(null,err)
                ...
                if(inputs.type == 'export'){
                    var fields = ['date','sum_live_times','sum_revence','sum_UV','sum_follow_count','sum_gift_count','sum_count','user_id','loginname','platform','anchor_group','admin_remark','regist_time' ]
                    var fieldNames = ['date','Live Times','Gift Value','UV','Follow Count','Gift Count','Live Count','UserId','Loginname','Platform','Anchor Group','Admin Remark','Regist Time' ]
                    ...
                    cms_report_db.LiveLogsDayReport.count({}, function(err,count){
                        if(err) cb(null,err)
